...
def get_missing_fields(user: BotUser):
    ...
    if user.budget is None:
        missing["budget"] = texts["indoor_budget"] if user.space_type == SpaceType.INDOOR else texts["outdoor_budget"]
    ...
    if user.style is None:
        missing["style"] = texts["indoor_style"] if user.space_type == SpaceType.INDOOR else texts["outdoor_style"]
